...
void merge_kurt_files_V2(const int startfile=0, const int endfile=1){
  ...
  */
  ...
  //output file:
  TFile f(Form("trig_merge_crosscheck_purdueforests_akpu3pf_%d.root",endfile),"RECREATE");
...
*/
  ...
  */
  ...
  TH1F* hpPb_100 = new TH1F("hpPb_100","",1000,0,1000);
  TH1F* hpPb_80 = new TH1F("hpPb_80","",1000,0,1000);
  TH1F* hpPb_60 = new TH1F("hpPb_60","",1000,0,1000);
  TH1F* hpPb_40 = new TH1F("hpPb_40","",1000,0,1000);
  TH1F* hpPb_20 = new TH1F("hpPb_20","",1000,0,1000);
  TH1F* hpPb_MB = new TH1F("hpPb_MB","",1000,0,1000);
  ...
  */
  ...
  TH1F* hpPb_Kurt100_nocut = new TH1F("hpPb_Kurt100_nocut","",1000,0,1000);
  TH1F* hpPb_Kurt80_100_nocut = new TH1F("hpPb_Kurt80_100_nocut","",1000,0,1000);
  TH1F* hpPb_Kurt60_80_nocut = new TH1F("hpPb_Kurt60_80_nocut","",1000,0,1000);
  TH1F* hpPb_Kurt40_60_nocut = new TH1F("hpPb_Kurt40_60_nocut","",1000,0,1000);
  TH1F* hpPb_Kurt20_40_nocut = new TH1F("hpPb_Kurt20_40_nocut","",1000,0,1000);
  TH1F* hpPb_KurtComb_nocut = new TH1F("hpPb_KurtComb_nocut","",1000,0,1000);
...
*/
      ...
      double maxtriggerPt=0;
      double topPt=0;
      for(int ii=0; ii<5; ii++){
        if(trgDec[ii]){
          for(int isize=0; isize<trigObjSize[ii]; isize++){
            double tPt = HLT_PAJet_NoJetID_v1_trigObject[ii]->at(isize).pt();
            if(tPt > maxtriggerPt && (tPt-TMath::Floor(tPt)) > 0.0001){ // check against integer (L1) trigger pT values
              maxtriggerPt = tPt;
            }
          }
        }      
      ...
      if(jet100 && maxtriggerPt>=100) {weight = jet100_p; flag = 5;}
      if(jet80 && maxtriggerPt>=80 && maxtriggerPt<100) {weight = jet80_p; flag = 4;}
      if(jet60 && maxtriggerPt>=60 && maxtriggerPt<80) {weight = jet60_p; flag = 3;}
      if(jet40 && maxtriggerPt>=40 && maxtriggerPt<60) {weight = jet40_p; flag = 2;}
      if(jet20 && maxtriggerPt>=20 && maxtriggerPt<40) {weight = jet20_p; flag = 1;}

      //get the trigger turn on curves here: without any selection cuts 
      for (int j = 0; j < nrefe3; j++)
      {
        if(jetMB) hpPb_MB->Fill(pt3[j],L1_MB_p*jetMB_p);
        if(jet20) hpPb_20->Fill(pt3[j],jet20_p);
        if(jet40) hpPb_40->Fill(pt3[j],jet40_p);
        if(jet60) hpPb_60->Fill(pt3[j],jet60_p);
        if(jet80) hpPb_80->Fill(pt3[j],jet80_p);
        if(jet100) hpPb_100->Fill(pt3[j],jet100_p);


  //now do it for kurt's method
        //if(fabs(eta3[j]+etashift)<1){

          if(flag == 5) hpPb_Kurt100_nocut->Fill(pt3[j],weight);
          if(flag == 4) hpPb_Kurt80_100_nocut->Fill(pt3[j],weight);
          if(flag == 3) hpPb_Kurt60_80_nocut->Fill(pt3[j],weight);
          if(flag == 2) hpPb_Kurt40_60_nocut->Fill(pt3[j],weight);
          if(flag == 1) hpPb_Kurt20_40_nocut->Fill(pt3[j],weight);
...
*/
	...
	//now do it for kurt's method
       if(fabs(eta3[j]+etashift)<1){
         if(flag == 5) hpPb_Kurt100->Fill(pt3[j],weight);
         if(flag == 4) hpPb_Kurt80_100->Fill(pt3[j],weight);
         if(flag == 3) hpPb_Kurt60_80->Fill(pt3[j],weight);
         if(flag == 2) hpPb_Kurt40_60->Fill(pt3[j],weight);
         if(flag == 1) hpPb_Kurt20_40->Fill(pt3[j],weight);
  ...
  */
  ...
  hpPb_KurtComb_nocut->Add(hpPb_Kurt100_nocut);
  hpPb_KurtComb_nocut->Add(hpPb_Kurt80_100_nocut);
  hpPb_KurtComb_nocut->Add(hpPb_Kurt60_80_nocut);
  hpPb_KurtComb_nocut->Add(hpPb_Kurt40_60_nocut);
  hpPb_KurtComb_nocut->Add(hpPb_Kurt20_40_nocut);
  hpPb_KurtComb_nocut->Print();
